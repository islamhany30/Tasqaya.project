...
export class TaskSchedulerService {
  ...
  constructor(
    ...
    @InjectRepository(TaskWorker) private readonly taskWorkerRepo: Repository<TaskWorker>,
  ...
  @Cron('*/5 * * * *')
  async handleTaskStatusUpdates() {
    this.logger.log('Starting task status and worker metrics sync...');
    const now = new Date();

    // 1. تحديث المهام إلى IN_PROGRESS
    await this.taskRepo
      .createQueryBuilder()
      .update(Task)
      .set({ status: TaskStatusEnum.IN_PROGRESS })
    ...
    // 3. جلب المهام التي انتهت في الـ 5 دقائق الأخيرة فقط
    // هذا يمنع تكرار الحسابات في كل دورة للـ Cron
    const fiveMinutesAgo = new Date(now.getTime() - 5 * 60 * 1000);
    const completedTasks = await this.taskRepo.find({
      where: {
        status: TaskStatusEnum.COMPLETED,
        endDate: Between(fiveMinutesAgo, now),
      },
    });

    if (!completedTasks.length) return;

    // 4. معالجة الـ Rating والـ Attendance
    const taskIds = completedTasks.map(t => t.id);
    const allLevels = await this.workerLevelRepo.find({ order: { minScore: 'ASC' } });

    const taskWorkers = await this.taskWorkerRepo.find({
      where: { task: { id: In(taskIds) }, confirmationStatus: WorkerConfirmationStatusEnum.CONFIRMED },
      relations: ['worker', 'task'],
    });

    // حساب الـ Attendance للمهام المكتملة حديثاً
    const attendanceCounts = await this.attendanceRepo
      .createQueryBuilder('a')
      .select('a.workerId', 'workerId')
      .addSelect('a.taskId', 'taskId')
      .addSelect('COUNT(*)', 'count')
      .where('a.taskId IN (:...taskIds)', { taskIds })
      .andWhere('a.status = :status', { status: AttendanceStatusEnum.PRESENT })
      .groupBy('a.workerId')
      .addGroupBy('a.taskId')
      .getRawMany();

    const attendanceMap = new Map(attendanceCounts.map(r => [`${r.workerId}-${r.taskId}`, Number(r.count)]));

    // تحديث بيانات العمال
    for (const tw of taskWorkers) {
      const { worker, task } = tw;
      const totalDays = Math.ceil((new Date(task.endDate).getTime() - new Date(task.startDate).getTime()) / (1000 * 60 * 60 * 24)) + 1;
      const presentDays = attendanceMap.get(`${worker.id}-${task.id}`) || 0;
      ...
      // معادلة التقييم
      const newAvg = (oldAvg * (worker.completedTasks || 0) + currentTaskRate) / ((worker.completedTasks || 0) + 1);
      const newScore = Math.max(0, (worker.score || 0) + (presentDays === 0 ? -5 : 5));
      const newLevel = allLevels.filter(l => l.minScore !== null && newScore >= l.minScore).sort((a, b) => b.minScore - a.minScore)[0] || worker.level;
    ...
    this.logger.log(`Worker metrics sync completed for ${completedTasks.length} tasks.`);
  }
}
