 ...
 async updateProfileImage(workerId: number, imageFile: Express.Multer.File) {
  ...
  async getMyApplications(workerId: number, pagination: PaginationDto = new PaginationDto()): Promise<any> {
    ...
    const applications = await this.applicationRepository
      ...
      .leftJoinAndSelect('task.workerLevel', 'workerLevel')
      .leftJoinAndSelect('app.worker', 'worker')
      // ── Join TaskWorker to get confirmation status for this worker ──
      .leftJoinAndSelect(
        'task.taskWorkers',
        'tw',
    ...
    const appResponses = applications.map((app) => {
      ...
      // ── Pick this worker's TaskWorker record (if assigned) ──
      const taskWorkerRecord = app.jobPost.task.taskWorkers?.[0] ?? null;
      // ────────────────────────────────────────────────────────
      ...
      return {
        ...
        appliedAt: app.appliedAt,
        // ── Confirmation fields ──────────────────────────────
        assignmentType: taskWorkerRecord?.assignmentType ?? null,
        confirmationStatus: taskWorkerRecord?.confirmationStatus ?? null,
        // ────────────────────────────────────────────────────
        jobPost: {
          id: app.jobPost.id,
          eventName: app.jobPost.task.eventName,
